...
def extract_comprehensive_page_text(
    ...
    use_ocr: bool = True,
    dpi: int = 200,  # Higher DPI for better OCR quality (will be capped by page size limits)
    ...
    def _get_ocr_text(page: fitz.Page, client, processor_name: str, dpi: int) -> List[Dict]:
        ...
        try:
            # Calculate optimal DPI that stays within Google Document AI limits (10k pixels max)
            # Get page dimensions in points
            page_width = page.rect.width
            page_height = page.rect.height
            
            # Calculate max DPI that keeps both dimensions under 10000 pixels
            max_dimension = max(page_width, page_height)
            max_dpi = int((10000 / max_dimension) * 72)  # 72 is base DPI reference
            
            # Use requested DPI if it's within limits, otherwise use calculated max
            # Minimum 150 DPI for quality, maximum based on page size
            working_dpi = min(max(dpi, 150), max_dpi, 300)  # Cap at 300 but respect limits
            
            # Ensure we don't go below 72 DPI (minimum working DPI)
            working_dpi = max(working_dpi, 72)
            
            print(f"🔍 Using DPI: {working_dpi} (requested: {dpi}, max allowed: {max_dpi}, page: {page_width:.0f}x{page_height:.0f}pt)")
            
            # Create image with calculated DPI
            ...
            error_str = str(e)
            print(f"❌ OCR failed: {error_str}")
            
            # Check if it's a size limit error
            is_size_error = ("10k pixel" in error_str.lower() or 
                           "too large" in error_str.lower() or 
                           "400" in error_str or
                           "exceeds" in error_str.lower())
            
            if is_size_error:
                # Progressive DPI reduction fallback
                fallback_dpis = [150, 120, 96, 72]  # Progressive reduction
                for fallback_dpi in fallback_dpis:
                    try:
                        print(f"🔄 Trying fallback: DPI {fallback_dpi}...")
                        pix = page.get_pixmap(dpi=fallback_dpi)
                        img_data = pix.tobytes("png")

                        # Check if dimensions are within limits
                        if pix.width > 10000 or pix.height > 10000:
                            print(f"   ⚠️ Dimensions {pix.width}x{pix.height} still too large, trying next DPI...")
                            continue
                        ...
                        print(f"📄 Fallback OCR response: {len(doc_result.text)} chars, {len(doc_result.pages)} pages")
                        ...
                        if doc_result.pages:
                            ...
                            for token in pg.tokens:
                                ...
                                if text_content and len(text_content) > 0:
                                    v = token.layout.bounding_poly.vertices
                                    if len(v) >= 4:
                                        x0, y0 = float(v[0].x) * scale_x, float(v[0].y) * scale_y
                                        x1, y1 = float(v[2].x) * scale_x, float(v[2].y) * scale_y
                        ...
                        print(f"   ❌ Fallback DPI {fallback_dpi} failed: {e_fallback}")
                        continue  # Try next DPI
                
                # If all fallbacks failed, return empty but don't crash
                print(f"⚠️ All fallback DPI attempts failed, returning empty OCR results")
                return []
            
            else:
                # Not a size error, return empty
                print(f"⚠️ OCR failed with non-size error, returning empty results")
                return []
