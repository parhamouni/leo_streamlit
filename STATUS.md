...
| Comprehensive upfront damage probe — first cut: `--damage-probe-batch` mode + `_damage_probe_one`/`_damage_probe_batch` helpers, separate pre-Phase-1a pass | [`df199cd`](https://github.com/parhamouni/leo_streamlit/commit/df199cd) | **Superseded** by `09962fc` (consolidated into Phase 1a — one subprocess pass instead of two) |
| **Damage probe folded into Phase 1a's subprocess** — `_extract_one_phase1a` now runs `get_text("words"|"dict")` + `get_drawings()` inline alongside text+dims extraction, in the same SIGKILL-protected subprocess. Deletes `_damage_probe_*` helpers and the upfront probe loop. Cache key bumped `phase1a` → `phase1a_v2` so old narrow-probe entries don't bypass the broader checks. **Net code: −308 / +182 lines.** Suite 62/62 (8 Phase 1a probe tests). | [`09962fc`](https://github.com/parhamouni/leo_streamlit/commit/09962fc) | Live on `main` |
...
### Damaged-page hang in Phase 3 — RESOLVED 2026-05-09 (commits `df199cd` → `09962fc`)
...
**Root cause**: Phase 1a's subprocess isolation only probed `get_text("text")`. Phase 3 calls `get_text("words")`, `get_text("dict")`, `get_drawings()`, and `extract_vector_lines()` — *different* MuPDF code paths that can hang on a different damage class than what Phase 1a catches.

**Fix shipped — final form (`09962fc`)**: the broader probe runs **inside Phase 1a's existing per-page subprocess**, not as a separate pre-pass. `_extract_one_phase1a` in `ops/page_extractor.py` now does `get_text("text")` (capture), then `get_text("words")`, `get_text("dict")`, `get_drawings()` (probe-only, results discarded), then dims — all under the same SIGKILL-on-timeout protection. If any op throws or the subprocess gets killed, the page is reported `ok=False` with the failing op in the error message, gets added to `broken_pages`, and is filtered from every later phase by the existing `if pi in broken_pages: continue` guards. Cache key bumped `"phase1a"` → `"phase1a_v2"` so cache entries built when only `get_text("text")` was probed don't let damaged pages slip through. The Phase 3 `_phase3_extract_lines` mitigation was reverted; the inline `pdf_lines = ade.get_native_pdf_lines(doc[pi])` call is back.

**Why two commits**: `df199cd` first shipped the probe as a *separate* pre-Phase-1a pass that ran every MuPDF call in its own subprocess. Worked correctly but wasteful — every healthy page paid two subprocess forks and `get_text("text")` was called twice per page. `09962fc` consolidated it into Phase 1a's existing subprocess: one fork, one `get_text("text")`, broader probe ops appended. ~20–30 s faster on a 179-page PDF, −308 / +182 line diff.
...
**Verification**:
- pytest 62/62 green (54 prior + 8 Phase 1a probe tests in `tests/test_phase1a_probe.py`).
- Smoke on `subset_gold/selected_pages_no_annotations.pdf`: `init` no longer emits "Probing pages for damage…" — goes straight from "Opening PDF" → "PDF loaded: 5 pages" → Phase 1a. Cache-hit re-run completes in 69 s, 5/5 fence pages, no errors.
- **Live validation (post-deploy)**: Lone Mountain (179p) + Construction Documents (136p) re-upload running concurrently with `max_concurrent=2`. Phase 1a caught a damaged page on Lone Mountain at the 20 s timeout — `WARNING Phase 1a page 56: page extraction exceeded 20s — MuPDF recovery loop on damaged page — marking damaged and skipping` — and the worker continued through Phase 1b/1c/2 without wedging the API. Exactly the bug class that hung us this afternoon, caught and contained.
- **Memory health** during concurrent processing: cgroup peak 22.2 GB / `MemoryMax=32 GB` (well under), uvicorn `VmHWM` 11.86 GB then dropped to 2.78 GB after work completed (Python returned memory to OS — no leak indicators). 0 zombie children, 20 open fds, 53 GB system available.
